...
const PostCard = ({ allData }) => {
	...
	const profile = state?.user?.profilePic;
		...
		if (isVideoUrl(item)) {
			return (
				<View className="w-full h-full">
					...
					/>
					<TouchableOpacity
						className="absolute top-1 right-10 bg-white w-9 h-9 rounded-full flex items-center justify-center"
						onPress={toggleMute}
					>
						{isMuted ? (
	...
	const handleDeletePost = async (id) => {
		try {
			...
			if (currentPost?.postType === 'quiz') {
				const { data } = await axios.delete(`/post/quiz/delete-post/${id}`);
				setLoading(false);
		...
		<View className="h-full w-full my-[5px]">
			...
			{allData
				...
				.map((post, i) => {
					...
					return (
						<View
							className="w-full mb-1 border-[1px] border-slate-300 rounded-md"
							key={i}
						>
							<View className="w-full flex-row justify-between items-center p-1">
								<View className="flex-row items-center space-x-2">
									...
									<View className="space-y-1">
										<TouchableOpacity>
											<Text className="font-medium tracking-wider">
										...
										<View className="flex flex-row items-center space-x-2">
											...
											<Text>
												{moment(post?.createdAt).isBefore(
							...
							{selectPostMenu[postId] && (
								<Animated.View
									...
									className="w-full h-auto flex-row items-center justify-between py-[2px] px-1 my-1"
								>
									{post?.postedBy?._id === userId && (
										<TouchableOpacity
											className="px-3 flex-row items-center h-9 space-x-1 rounded-full bg-slate-200"
											onPress={() => {
									...
									{post?.postedBy?._id === userId && (
										<TouchableOpacity
											className="px-3 flex-row items-center h-9 space-x-1 rounded-full bg-slate-200"
											onPress={() => handleDeleteAlert(postId)}
										>
							...
							{(post?.postType === "videos" ||
								...
								post?.postType === "videowithquiz") && (
									<View className="w-full h-96 my-1">
										...
										<TouchableOpacity
											className="absolute top-1 right-1 bg-white w-9 h-9 rounded-full flex items-center justify-center"
										...
										{post?.images.length > 1 && (
											<View className="absolute top-1 left-1 bg-white w-[42px] h-[26px] rounded-full flex items-center justify-center">
												<Text className="tracking-widest mb-[1px]">
													{activeSlide + 1}/{post?.images.length}
												</Text>
											</View>
										)}
									</View>
